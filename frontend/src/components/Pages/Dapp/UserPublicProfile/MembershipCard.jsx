...
const MembershipCard = ({
  ...
  if (network === 80001) {
    contractAddress = process.env.REACT_APP_sucs_SMART_CONTRACT;
  } else if (network === 137) {
    contractAddress = process.env.REACT_APP_sucs_SMART_CONTRACT_MAINNET;
  }
  ...
  return (
    <div className="py-10 px-10 text-center">
      ...
      <div className="py-5">
        ...
        <p className="uppercase text-sucsGray text-sm tracking-widest">
          <span className="text-sucsBlue/70 font-semibold">MATIC</span> per
          month
        </p>
...
const LoginButton = () => {
  ...
  const login = async () => {
    if (!isAuthenticated) {
      try {
        await authenticate({ signingMessage: "Log in to sucs" });
