...
const DappSidebar = () => {
  ...
  return (
    ...
    >
      <div className="cursor-pointer text-sucsBlue">
      ...
      <div className="rounded-lg flex flex-col gap-2 pb-5">
        <p className="w-3/4 text-left text-sm text-sucsBlue">
      ...
      {sidebarLinks.map((eachItem) => (
        <NavLink
          key={eachItem.name}
          to={eachItem.link}
          className="flex gap-2 items-center hover:text-sucsBlue cursor-pointer"
        >
          {eachItem.iconName}
          <p>{eachItem.name}</p>
        </NavLink>
      ...
      <div className="font-semibold text-lg text-sucsBlue cursor-pointer">
